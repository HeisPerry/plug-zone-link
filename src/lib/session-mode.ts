 ...
 * "Remember me" support.
 *
 * The auth client always stores the session in localStorage so refreshes work.
 * When the user opts OUT of being remembered we record that choice and tag the
 * browser session (sessionStorage). On the next full launch the tag is gone,
 * so the app signs the stored session out instead of restoring it.
 */
...
/**
 * Returns true when a stored session should be discarded because the user
 * chose not to be remembered and this is a fresh browser launch.
 */
